...
class Ruleset:
    ...
    def managed_dirs(self) -> set[str]:
        ...
        return set(self.mime_categories) | {OTHERS, "DUPLICATES"} | set(self.themes)


def load_ruleset(directory: Path) -> tuple[Ruleset, str | None]:
    """Build a :class:`Ruleset` from defaults + optional external config.

    Returns ``(ruleset, message)`` where ``message`` is a human-readable note
    (config loaded / config error) or ``None`` when no config file is present.
    Never raises on a bad config file — reports the problem and uses defaults.
    """
    ruleset = Ruleset()
    ...
    try:
        ...
        return ruleset, f"Invalid {CONFIG_FILE}, using defaults: {exc}"
    ...
    return ruleset, f"Configuration loaded from {CONFIG_FILE}"
